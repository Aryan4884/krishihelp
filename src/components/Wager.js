...
export const Wager = ({ addJob }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!data.firstname||!data.lastname||!data.email||!data.contactNo||!data.address||!data.District||!data.state||!data.pincode||!data.Wagers||!data.work||!data.amout) {
    ...
    addJob(data);
    fetch("http://localhost:8080/wager", {
      method: "POST",
      crossDomain: true,
      headers: {
